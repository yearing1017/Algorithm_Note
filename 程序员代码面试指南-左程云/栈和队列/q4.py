...
class DogCatQueue:
    ...
    def add(self, pet):
        count = len(self.dog_queue) + len(self.cat_queue)
        if pet.get_pet_type() == 'dog':
            self.dog_queue.append(DogCatCounter(pet, count))
        else:
            self.cat_queue.append(DogCatCounter(pet, count))
...
if __name__ == '__main__':
    ...
    print('依次弹出所有实例：')
    print(dog_cat_queue.isEmpty())
    while not dog_cat_queue.isEmpty():
        print(dog_cat_queue.pollAll().get_pet_type())
    print('===============')
    '''
    print('依次弹出所有dog实例：')
